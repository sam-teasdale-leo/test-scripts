...
TOKEN_EXCHANGE_RESPONSE=$(curl -s -X 'POST' 'https://verus-kubernetes-ca.verus-development.development.threatdeterrence.com/api/v2/authentication/token' \
  -H 'accept: application/json' \
  -H 'Content-Type: application/json' \
...
USER_TOS_RESPONSE=$(curl -s -X 'GET' "https://verus-kubernetes-ca.verus-development.development.threatdeterrence.com/api/v1/users/${USER_ID}/terms-of-service" \
...
PRESIGNED_URL=$(echo "$USER_TOS_RESPONSE" | jq -r '.["terms-of-service"]')
