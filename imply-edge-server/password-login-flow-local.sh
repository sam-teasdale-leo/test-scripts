...
CODE=$(curl -s -X POST 'http://localhost:4000/api/v1/authentication/login' \
...
RESPONSE=$(curl -s -X POST 'http://localhost:4000/api/v1/authentication/token' \
...
curl -s -X POST 'http://localhost:4000/api/v1/authentication/token-refresh' \
  -H 'accept: application/json' \
  -H 'Content-Type: application/json' \
  -d "{
  \"token\": \"$TOKEN\",
