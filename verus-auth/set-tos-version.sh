...
echo
echo "Step 2: Set TOS Version"
echo

SET_TOS_RESPONSE=$(curl -s -X 'POST' \
  "https://verus-auth-development.threatdeterrence.com/api/v2/users/$USER_ID/accept-tos?passwordless_login_code=$CODE&user_id=$USER_ID" \
  -H 'accept: application/json' \
  -H "Content-Type: application/json" \
  -d '{
  "tos_version": "2025.1.0"
}')

echo "$SET_TOS_RESPONSE" | jq .
